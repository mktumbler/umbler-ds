...
- orientation: \`horizontal\` | \`vertical\` — padrão: \`horizontal\``,
  ...
  'section-header': `Cabeçalho canônico de seção (eyebrow + h2 56px + sub). Use ao compor seções fora dos blocks que já trazem header próprio (FAQ/Hero). Instale: \`npx shadcn add https://umbler-ds.vercel.app/r/section-header\`
\`\`\`tsx
...
/>
\`\`\`
- Props: eyebrow?, headline (obrigatório), sub?, align: 'center' (default) | 'left'`,
